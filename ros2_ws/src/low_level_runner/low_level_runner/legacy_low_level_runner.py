"""Legacy low-level runner kept from the pre-Kiwi-controller implementation."""

# This module is retained for reference while the active ROS entry point moves to
# kiwi_drive_controller.py. It expects the old B_Cubed.main.HardwareInterface
# import path from the motors_test branch.
...
class LowLevelController(Node):
    def __init__(self):
        ...
        # Latest command from ROS — [x_velocity, y_velocity, angular_velocity,
    ...
    def control_loop(self):
        ...
        self.top_left_speed = (
            float(np.dot(self.velocity, TOP_LEFT_VEC)) + self.angular_velocity
        )
        self.top_right_speed = (
            float(np.dot(self.velocity, TOP_RIGHT_VEC)) + self.angular_velocity
        )
        self.bottom_speed = (
            float(np.dot(self.velocity, BOTTOM_VEC)) + self.angular_velocity
        )
