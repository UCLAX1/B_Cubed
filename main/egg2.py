...
def find_motor_angles(pitch, roll, desired_angle):
    ...
    if roll != 0:
        lazy_susan = math.acos(n1/math.sqrt(n1**2 + n2**2))
    
    if roll < 0:
        lazy_susan = -lazy_susan
        arm = -arm

    if (abs(lazy_susan) > np.pi / 4):
        arm = -arm
        if(lazy_susan > 0):
...
while True:
    if imu.IMURead():
        ...
        arm_angle, lazy_susan_angle, head_angle = find_motor_angles(fusionPose[0], fusionPose[1], 0.0)

        # print(f"angles: arm={np.rad2deg(arm_angle):6.1f} lazy_susan={np.rad2deg(lazy_susan_angle):6.1f} head={np.rad2deg(head_angle):6.1f}")


        # print(f"accel: z={accel[0]:6.2f} -y={accel[1]:6.2f} -x={accel[2]:6.2f}")

        print(f"accel: x={-accel[2]:10.2f} y={-accel[1]:10.2f}")



        # print(f"Orientation: R={r2d(fusionPose[0]):6.1f} P={r2d(fusionPose[1]):6.1f} Y={r2d(fusionPose[2]):6.1f}")
