# CAN IN CASE WE HAVE TO WORK WITH CAN
# https://canable.io/getting-started.html

# ALSO HAS CAN
# https://docs.revrobotics.com/brushless/spark-max/control-interfaces

# controlling spark max over can
# https://www.reddit.com/r/FRC/comments/1i1ovyx/control_spark_max_with_microcontroller_over_can/

# bruin bear reference
# https://github.com/UCLAX1/BruinBear/blob/motor-testing/Motor%20Test/amador_test.py

# from https://canable.io/getting-started.html
import can
import time

Duty_Cycle_Set = 0x2050080
Speed_Set = 0x2050480
Smart_Velocity_Set = 0x20504C0
Position_Set = 0x2050C80
Voltage_Set = 0x2051080
Current_Set = 0x20510C0
Smart_Motion_Set = 0x2051480
# Position_Set = 0x2050C80
Heartbeat_Set = 0x2052C80
Device_Specific_Heartbeat = 0x2052480

CAN_EFF_FLAG = 0x80000000

status_0 = 0x2051800
status_1 = 0x2051840
status_2 = 0x205B880
status_3 = 0x20518C0
status_4 = 0x2051900

# // Heartbeat Frame
HEARTBEAT_ID = 0x2052C80
HEARTBEAT_DATA = [ 255, 255, 255, 255, 255, 255, 255, 255 ]
HEARTBEAT_SIZE = 8

# bus = can.interface.Bus(bustype='socketcan', channel='can0', bitrate=1000000)
# bus = can.interface.Bus(channel='COM5', interface='slcan', bitrate=500000)

device_id = 1

# // Control Frame
CONTROL_SIZE = 8

# // Status Frame
STATUS_SIZE = 2

print("CAN CONTROLLER")


import struct

class CANDevice:
    # example of channels: "COM5", "COM9"
    # example of interface: "slcan"
    def __init__(self, device_id=1, channel="COM5", interface="slcan", bitrate=1000000):
        self.device_id = device_id
        self.channel = channel
        self.interface = interface
        self.bitrate = bitrate
        self.bus = can.interface.Bus(channel=channel, interface=interface, bitrate=bitrate)

    def send_speed_set(self, speed_rpm):
        """Send Speed Set command to motor controller with correct Extended ID"""

        # Compute the extended ID correctly
        ext_id = (Smart_Velocity_Set + self.device_id) | 0x80000000  # Ensure Extended Frame Format

        # Convert speed setpoint to IEEE 754 floating point (4 bytes, little-endian)
        speed_data = struct.pack('<f', speed_rpm)

        # Construct CAN Data (8 bytes)
        data = bytearray(8)
        data[:4] = speed_data  # Target Speed (bytes 0-3)
        data[4] = 0  # Aux[0]
        data[5] = 0  # Aux[1]
        data[6] = 0  # Reserved
        data[7] = 0  # Reserved

        # Create and send CAN message
        msg = can.Message(arbitration_id=ext_id, is_extended_id=True, data=data)

        try:
            self.bus.send(msg)
            print(f"Sent Speed Set command (ID: {hex(ext_id)}, Speed: {speed_rpm} RPM)")
            print("control data sent:", msg)
        except can.CanError as e:
            print("Error sending Speed Set command:", e)


    def send_heartbeat(self):
        heartbeat_id = (Heartbeat_Set + self.device_id) | 0x80000000
        heartbeat_data = [0xFF] * 8
        msg = can.Message(arbitration_id=heartbeat_id, is_extended_id=True, data=heartbeat_data)
        self.bus.send(msg)
        print(f"Sent Heartbeat to device {self.device_id} with ID {hex(heartbeat_id)}")
        print("heartbeat data sent:", msg)


    def send_device_specific_heartbeat(self):
        """Sends the special 'internal heartbeat' frame that makes the motor actually move"""
        internal_heartbeat_id = (Device_Specific_Heartbeat + self.device_id) | 0x80000000
        data = [0xFF] * 8
        msg = can.Message(arbitration_id=internal_heartbeat_id, is_extended_id=True, data=data)
        self.bus.send(msg)
        # print(f"Sent internal heartbeat to device {self.device_id} (ID: {hex(internal_heartbeat_id)})")


    def send_duty_cycle(self, percent_output):
        ext_id = (Duty_Cycle_Set + self.device_id) | 0x80000000
        duty_data = struct.pack('<f', percent_output)  # e.g., 0.5 for 50%
        data = bytearray(8)
        data[:4] = duty_data
        msg = can.Message(arbitration_id=ext_id, is_extended_id=True, data=data)
        self.bus.send(msg)
        # print(f"Sent Duty Cycle command: {percent_output * 100:.1f}%")


    def send_position_set(self, position_rotations):
        """Send Position Set command to SPARK MAX"""
        ext_id = (Position_Set + self.device_id) | 0x80000000

        # Pack position (float) as little-endian
        pos_data = struct.pack('<f', position_rotations)

        # Build 8-byte payload
        data = bytearray(8)
        data[:4] = pos_data  # Position (rotations)
        data[4] = 0  # Aux 0
        data[5] = 0  # Aux 1
        data[6] = 0  # Reserved
        data[7] = 0  # Reserved

        msg = can.Message(arbitration_id=ext_id, is_extended_id=True, data=data)
        try:
            self.bus.send(msg)
            print(f"Sent Position Set command (ID: {hex(ext_id)}, Position: {position_rotations} rotations)")
        except can.CanError as e:
            print("Error sending Position Set command:", e)


    def get_encoder_position(self, timeout=1.0):
        """Waits for a status 2 frame and extracts encoder position (in rotations)"""
        status_2_id = (0x205B880 + self.device_id)

        # Listen for messages on the bus
        print(f"Waiting for position frame from device {self.device_id}...")
        start_time = time.time()

        while time.time() - start_time < timeout:
            msg = self.bus.recv(timeout=timeout)
            if msg is None:
                continue
            if msg.arbitration_id == status_2_id and msg.is_extended_id:
                # Extract position from first 4 bytes
                position_bytes = msg.data[:4]
                position = struct.unpack('<f', position_bytes)[0]
                print(f"Got position: {position:.4f} rotations")
                return position

        print("Timed out waiting for position frame.")
        return None


    def set_status_frame_period(self, frame_base_id, period_ms):
        """
        Set the transmission period of a given status frame.
        `frame_base_id`: one of status_0, status_1, ..., status_4
        `period_ms`: how often to send, in milliseconds
        """
        ext_id = (frame_base_id + self.device_id) | CAN_EFF_FLAG
        # data = struct.pack('<H', period_ms)
        data = [0x14, 0x00]
        msg = can.Message(arbitration_id=ext_id, is_extended_id=True, data=data)
        try:
            self.bus.send(msg)
            print(f"Requested Status Frame {hex(ext_id)} at {period_ms}ms")
        except can.CanError as e:
            print("Error sending status frame config:", e)


    def read_can_once(self):
        """
        Reads and prints one CAN frame if available (non-blocking).
        Call this inside your main loop.
        """
        msg = self.bus.recv(timeout=0.001)  # Small timeout so it doesn’t block
        if msg is not None:
            if msg.arbitration_id & 0xFFFF == 0xb841:
                print(f"CAN Received | ID: {hex(msg.arbitration_id)}  "
                      f"{'EXT' if msg.is_extended_id else 'STD'}  "
                      f"DL: {msg.dlc}  Data: {' '.join(f'{b:02X}' for b in msg.data)}")

class Motor(CANDevice):
    def __init__(self, *args, **kwargs):
        super(Motor, self).__init__(*args, **kwargs)


device = Motor()

# time.sleep(2)
# send_position_set(1, 1.0)
device.send_position_set(1.0)

while True:
    # send_speed_set(1, 1000.0)
    device.send_duty_cycle(0.1)
    # send_position_set(1, 1.0)
    device.read_can_once()
    # send_position_set(1, 1.0)
    device.send_device_specific_heartbeat()

    # set_status_frame_period(1, status_2, 20)

    time.sleep(0.02)  # Send every second

#
# msg = can.Message(arbitration_id=0x01, data=[0x01, 0x02, 0x03, 0x04])
# bus.send(msg)
# print("Message sent!")
# # Receive a CAN message
# while (True):
#     for msg in bus:
#         print(msg.data)
