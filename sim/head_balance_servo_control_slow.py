"""
IMU reader with smoothed servo control - SLOW VERSION.

Reads IMU data, calculates target motor angles, and moves the servos to
balance very slowly for testing. The arm servo uses open-loop PD command damping.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Balance servos from IMU pitch/roll data (SLOW VERSION)."
    )
    parser.add_argument(
        "--duration",
        type=float,
...
def main(
    ...
    log("Starting head balance servo control (SLOW VERSION).")
