...
try:
    ...
    if DEBUG:
        print(
            f"pigpio not available ({e}); falling back to default. "
        ...
        while True:
            if imu.IMURead():
                ...
                if dt >= servo_update_interval:
                    ...
                    arm_raw = angle_to_servo_value(damped_arm_target, SERVO_RANGE_DEG)
                    lazy_raw = angle_to_servo_value(lazy_susan_target, SERVO_RANGE_DEG)
                    ...
                    arm_cmd = arm_cmd_last + clamp(
                        arm_cmd - arm_cmd_last, -arm_max_step, arm_max_step
                    )
                    lazy_cmd = lazy_cmd_last + clamp(
                        lazy_cmd - lazy_cmd_last, -cont_max_step, cont_max_step
                    )
                    head_cmd = head_cmd_last + clamp(
                        head_cmd - head_cmd_last, -cont_max_step, cont_max_step
                    )
                    ...
                    arm_cmd_last, lazy_cmd_last, head_cmd_last = (
                        arm_cmd,
                        lazy_cmd,
                        head_cmd,
                    )
                ...
                if DEBUG and now - last_print >= print_interval:
                    ...
                    print(
                        f"Cmd: arm={arm_cmd_last:+.3f}  lazy={lazy_cmd_last:+.3f}  head={head_cmd_last:+.3f}"
                    )
                    print("-" * 70)
